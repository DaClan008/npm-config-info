 ...
 * @returns {folderInformation}
 */
function getFolders(config) {
    /** @type {folderInformation } from "./typings/objects";} */
    let resultObj = {};
 ...
 * @private
 * @param {configObject} config the Config object.
 * @param {authorInformation} author the Author information.
 * @returns {initObject}
 */
...
const globState = {
    ...
    NotInitialized: "getGlobal function not yet called.",
    /** The global file could not be found. */
    NotFound: "can't find configFile.",
...
class ConfigInfo {
    ...
    constructor(data, includeGlobal) {
         ...
         * @type {globState}
         */
        this.globalState = globState.NotInitialized;
        if (includeGlobal) this.getGlobal();
            ...
            this.global = globState.NotInitialized;
        }
    }
    ...
    getGlobal(configFile) {
        if (this.folders.globalconfig || configFile) {
            ...
            if (success) {
                ...
                this.globalState = globState.Initialized;
                this.global = new ConfigInfo(data.toString(), false);
                return true;
            } else {
                this.globalExist = false;
                this.globalState = globState.NotFound;
                this.global = globState.NotFound;
                return false;
            }
        }
    }
